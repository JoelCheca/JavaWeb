 ...
 */
@WebServlet({"/VentaController","/postProcesarVenta",  "/postModificarPrecio"})
public class VentaController extends HttpServlet {
	...
	private void postProcesarVenta(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			...
			double dcto =Double.parseDouble( request.getParameter("dcto"));

			
			// Proceso
			ServicesBook service = new ServicesBook();
			service.registraVenta(cliente, idempleado,idpublicacion , cantidad, precio, total, impuesto, subtotal, dcto);
			msg.setEstado(1);
