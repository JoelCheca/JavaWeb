...
import pe.edu.uni.bookstore.service.ServicesBook;
	...
	@WebServlet({"/traerCombos","/traerPrecio","/traerCombos2","/traerCombos3"})
	public class ComboController  extends HttpServlet {
		...
		private void traerCombos3(HttpServletRequest request, HttpServletResponse response) throws IOException {
			String descripcion = request.getParameter("descripcion");
			// Proceso
			ConsultasService service = new ConsultasService();
			List<Map<String, ?>> lista = service.comboPublicacionesTipo(descripcion);
			// Reporte
			Gson gson = new Gson();
