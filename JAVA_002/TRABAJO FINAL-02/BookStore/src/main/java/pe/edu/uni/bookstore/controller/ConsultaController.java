...
@WebServlet({"/ConResumen1","/ConResumen2", "/listarPublicaciones" ,"/ultimasVentas","/calcular" })
public class ConsultaController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "/ultimasVentas":
			getultimasVentas(request, response);
			break;
			
		case "/calcular":
			getCalcular(request, response);
			break;
	...
	private void getultimasVentas(HttpServletRequest request, HttpServletResponse response) throws IOException {
		// Proceso
	...
	private void getCalcular(HttpServletRequest request, HttpServletResponse response) 
			throws IOException {
		// Parametro
		...
		// Proceso
		ServicesBook service = new ServicesBook();
		List<Map<String,?>> lista = service.calcular(idpublicacion, cantidad);
		// Reporte
		Gson gson = new Gson();
		String strJson = gson.toJson(lista);
		HttpUtil.reporteJson(response, strJson);
